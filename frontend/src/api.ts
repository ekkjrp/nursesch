// API 클라이언트 — Python FastAPI 백엔드 REST API 호출 (v1.2)
...
export const shiftRequests = {
  ...
  update: (id: number, data: any) => request<any>(`/requests/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
...
export const logs = {
  pageView: (path: string) =>
    request<any>('/logs/page-view', { method: 'POST', body: JSON.stringify({ path, timestamp: new Date().toISOString() }) }).catch(() => {}),
};
...
export const holidays = {
  ...
  autoFill: (wardId: number, year: number) =>
    request<any[]>(`/holidays/auto-fill?ward_id=${wardId}&year=${year}`, { method: 'POST' }),
};
