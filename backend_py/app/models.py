...
class Nurse(Base):
    ...
    # 직급: HN(수간호사/관리자), CN(책임간호사), RN(평간호사), AN(보조간호사), PN(임시간호사)
    grade = Column(String(10), nullable=False, default="RN")
    sort_order = Column(Integer, default=0)          # 관리자 지정 정렬 순서
    is_night_dedicated = Column(Boolean, default=False)  # (deprecated) 나이트 전담 여부
    dedicated_shift = Column(String(5), nullable=True, default=None)  # 지정근무: D/E/N/M 또는 null
    weekday_preference = Column(String(5), nullable=True, default=None)  # 평일 선호 근무
    weekend_preference = Column(String(5), nullable=True, default=None)  # 주말 선호 근무
...
class Rule(Base):
    ...
    # 월간 최대 나이트 근무일수 (0=무제한)
    max_monthly_night_shifts = Column(Integer, default=0)
    # 제약 가중치 (JSON 문자열)
...
class NurseMonthlyLeave(Base):
    ...
    total_off_days_override = Column(Integer, nullable=True, default=None)  # null이면 자동 계산
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
