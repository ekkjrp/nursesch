"""제약 조건 검증기 — Scheduling Engine과 분리된 독립 모듈 (v1.2)"""
...
def validate_schedule(
    ...
    # PN 간호사는 모든 제약 검사 스킵
    pn_ids = {n["id"] for n in nurses if n.get("grade") == "PN"}
    ...
    for date in all_dates:
        ...
        for shift, col in [("D", "day"), ("E", "evening"), ("N", "night")]:
            workers = [
                ...
                if shifts.get(date) == shift and nid in nurse_map and nid not in pn_ids
            ]
    ...
    # HC-5 일반화: 지정근무 준수 (dedicated_shift 기반)
    for nid, shifts in schedule.items():
        if nid in pn_ids:
            continue
        ...
        # backward compat: is_night_dedicated가 있고 dedicated_shift 없으면 N 전담
        if not dedicated and nurse.get("is_night_dedicated"):
            dedicated = "N"
        if not dedicated:
            continue
        for date, st in shifts.items():
            # 지정근무가 있으면 해당 근무 + O/Y/X만 허용
            if st not in (dedicated, "O", "Y", "X"):
                violations.append(Violation(
                    type="HARD", rule="HC-5",
                    message=f"간호사 {nid}({dedicated}전담): {date} 허용되지 않은 근무({st})",
                    nurse_id=nid, date=date
                ))
