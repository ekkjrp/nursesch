...
- **Scheduling Engine** (`backend_py/app/engine/scheduler.py`) — SA 알고리즘 기반 자동 배정. 커버리지 우선 초기해 + 위반 지향 이웃해 생성
...
- **Charge 등급** (`HN`, `CN`): 근무조 책임자 역할. 각 근무(D/E/N)에 최소 1명 필요.
- **Action 등급** (`RN`): 실무 담당. 각 근무에 최소 인원 필요.
- **나이트 전담**: `is_night_dedicated=True`인 간호사는 N/O만 배정 가능.
...
| HC-5 | 나이트 전담 준수 | 나이트 전담 간호사는 N/O/Y/X만 배정 |
